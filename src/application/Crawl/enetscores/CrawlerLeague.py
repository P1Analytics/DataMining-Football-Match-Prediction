...
class CrawlerLeague(object):
    def __init__(self, league_name, league_data_stage):
        ...
        self.link_league_to_check = "http://football-data.mx-api.enetscores.com/page/xhr/standings/1%2F0%2F0%2F0%2F" \
                                    + self.league_data_stage + "%2F0/"
        log.debug("Check by league data stage [" + self.league_data_stage + "] if this is a managed league ["
                  + self.link_league_to_check + "]")
    ...
    def get_season(self):
        ...
        div_season = self.soup.find('div',
                                    {'class': 'mx-dropdown-container mx-flexbox mx-float-left mx-tournament-dropdown'})
